...
float getClosestDepth(vec2 uv){
    float closest = texture(depthtex1, uv).r;
    #if defined DISTANT_HORIZONS && !defined NETHER && !defined END
        #ifdef GBF
            float dhDepth = texture(dhDepthTex1, uv).r;
        #else
            float dhDepth = texture(dhDepthTex0, uv).r;
        #endif
        if(dhDepth < 1.0){
            vec4 dhViewPos = screenPosToViewPosDH(vec4(uv, dhDepth, 1.0));
...
float getMinDepth2x2(vec2 uv){
    #if defined DISTANT_HORIZONS && !defined NETHER && !defined END
        return getClosestDepth(uv);
    #else
        #if __VERSION__ >= 400
            vec4 d = textureGather(depthtex1, uv);
            return min4(d.x, d.y, d.z, d.w);
        #else
            ivec2 pix = ivec2(uv * viewSize);
            ivec2 base = (pix >> 1) << 1;
            ivec2 maxPix = ivec2(viewSize) - ivec2(1);
            float d0 = texelFetch(depthtex1, clamp(base + ivec2(0, 0), ivec2(0), maxPix), 0).r;
            float d1 = texelFetch(depthtex1, clamp(base + ivec2(1, 0), ivec2(0), maxPix), 0).r;
            float d2 = texelFetch(depthtex1, clamp(base + ivec2(0, 1), ivec2(0), maxPix), 0).r;
            float d3 = texelFetch(depthtex1, clamp(base + ivec2(1, 1), ivec2(0), maxPix), 0).r;
            return min4(d0, d1, d2, d3);
        #endif
    #endif
}
...
vec2 SSRT(vec3 viewPos, vec3 reflectViewDir, vec3 normalTex){
    ...
    #if defined SSR_HIZ && SSR_HIZ_STEPS > 0
        int hizSteps = min(int(REFLECTION_SAMPLES), SSR_HIZ_STEPS);
        float hizGrowth = max(REFLECTION_STEP_GROWTH_BASE, SSR_HIZ_STEP_GROWTH);
        for (int i = 0; i < hizSteps; ++i){
            ...
            float minDepth = getMinDepth2x2(testScreenPos.xy);
            if (testScreenPos.z > minDepth + SSR_HIZ_THICKNESS){
                // Rewind one step so the fine pass brackets the hit tightly.
                cumUnjittered -= curStep;
                curStep = max(curStep * 0.5, REFLECTION_STEP_SIZE);
                break;
            }
    ...
    int remainingSteps = int(REFLECTION_SAMPLES) - usedSteps;
    for (int i = 0; i < remainingSteps; ++i){
        ...
        float closest = getClosestDepth(testScreenPos.xy);
